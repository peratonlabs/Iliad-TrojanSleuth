...
from pruning import PruningTrojaiMitigationLLM
import utils
...
def run_test_mode(argv):
    ...
    jsonschema.validate(instance=config_json, schema=schema_json)
    model, tokenizer = prepare_model_and_tokenizer(argv.model)
    inference_model(argv.model, model, tokenizer)
    
def inference_model(model_dir, model, tokenizer):
    # load prompts json using huggingface dataset
    prompts_json_filepath = 'example_data.json'
    json_dataset = load_dataset('json', data_files=[prompts_json_filepath], split='train')

    class MyDataset(torch.utils.data.Dataset):
        def __len__(self):
            return len(json_dataset)

        def __getitem__(self, i):
            tensor_dict = json_dataset[i]
            user_message = dict()
            user_message['content'] = tensor_dict['messages'][0]['content']#['poisoned_content']
            user_message['role'] = 'user'
            prompt = tokenizer.apply_chat_template(conversation=[user_message], add_generation_prompt=True, tokenize=False)
            return prompt
    ...
    for out in tqdm.tqdm(pipe(ds, max_new_tokens=max_num_tokens, batch_size=batch_size, no_repeat_ngram_size=3, do_sample=True, pad_token_id=tokenizer.pad_token_id, eos_token_id=tokenizer.eos_token_id), total=len(ds)):
        #print(out[-1]['generated_text'])
        results_list.append(out[-1]['generated_text'])

    pipe.model.to('cpu')  # free gpu space
